...
/// @title  NFTStakerPriceScaled
/// @notice Decimal-normalizing standalone copy of `NFTStaker`. This contract
///         is byte-for-byte identical to `src/NFTStaker.sol` EXCEPT for an
///         immutable `priceScale` multiplier that normalizes the dispatcher's
///         mint `price` (denominated in the dispatcher's prime token) into
///         reward-token (phUSD) units before it is used as the NFT-value
///         proxy in `_recomputeSchedule`. With `priceScale == 1` this contract
///         is behaviorally identical to `NFTStaker`; for a 6dp USDC-priced
///         dispatcher paying 18dp phUSD, `priceScale == 1e12`.
///
///         WHY A COPY: the live `NFTStaker` deployed for the BalancerPoolerV2
///         dispatcher is audited and not retiring, so its source stays
///         byte-for-byte untouched. This contract is deployed ALONGSIDE the
///         original for the NudgeRatchet dispatcher (whose 6dp prime token
///         otherwise floor-divides the emission rate to zero).
///
...
///         only intended deltas are: the contract name, the `priceScale`
///         immutable + constructor guard/assignment, and the single
///         `latestPrice = latestPrice * priceScale;` multiply in
///         `_recomputeSchedule`.
///
...
///         for the full design.
contract NFTStakerPriceScaled is Ownable, Pausable, ReentrancyGuard, ERC1155Holder, IPausable {
