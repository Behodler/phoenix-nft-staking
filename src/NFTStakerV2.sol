...
///         for the full design.
contract NFTStakerV2 is Ownable, Pausable, ReentrancyGuard, ERC1155Holder, IPausable {
    ...
    /// @notice Address authorised to call stakeFor/unstakeFor and (jointly with
    ///         the owner) withdrawRewardToken. Settable by owner via
    ///         setMigrator. address(0) disables all migrator-gated functions.
    ...
    modifier onlyPauser() {
        require(msg.sender == pauser, "NFTStakerV2: caller is not pauser");
        _;
    }

    modifier onlyMigrator() {
        require(msg.sender == migrator && migrator != address(0), "NFTStakerV2: caller is not migrator");
        _;
    }

    modifier onlyOwnerOrMigrator() {
        require(
            msg.sender == owner() || (msg.sender == migrator && migrator != address(0)),
            "NFTStakerV2: caller is not owner or migrator"
        );
        _;
    }
    ...
    constructor(
        ...
        require(address(_stakedToken) != address(0), "NFTStakerV2: zero staked token");
        require(address(_rewardToken) != address(0), "NFTStakerV2: zero reward token");
        require(address(_nftMinter) != address(0), "NFTStakerV2: zero nft minter");
    ...
    function setMigrator(address newMigrator) external onlyOwner {
        emit MigratorChanged(migrator, newMigrator);
    ...
    function setNFTMinter(INFTSupply newMinter) external onlyOwner {
        require(totalStaked == 0, "NFTStakerV2: stake outstanding");
        require(address(newMinter) != address(0), "NFTStakerV2: zero nft minter");
    ...
    function setTargetAPY(uint256 newAPY) external onlyOwner {
        require(newAPY <= MAX_TARGET_APY, "NFTStakerV2: APY too high");
        _updatePool();
    ...
    function topUp(uint256 amount) external onlyOwner {
        require(amount > 0, "NFTStakerV2: zero topUp");
        _updatePool();
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                pending = _safePay(msg.sender, pending);
    ...
    function unstake(uint256 amount) external nonReentrant whenNotPaused {
        require(amount > 0, "NFTStakerV2: zero unstake");
        UserInfo storage user = users[msg.sender];
        require(user.amount >= amount, "NFTStakerV2: insufficient stake");
        _syncBudget();
        uint256 pending = (user.amount * accRewardPerShare) / ACC_PRECISION - user.rewardDebt;
        if (pending > 0) {
            pending = _safePay(msg.sender, pending);
    ...
    function claim() external nonReentrant whenNotPaused {
        ...
        uint256 pending = (user.amount * accRewardPerShare) / ACC_PRECISION - user.rewardDebt;
        if (pending > 0) {
            uint256 paid = _safePay(msg.sender, pending);
    ...
    // ---------------------------------------------------------------------
    // Migrator entrypoints
    // ---------------------------------------------------------------------

    function stakeFor(address beneficiary, uint256 amount) external nonReentrant whenNotPaused onlyMigrator {
        ...
        if (user.amount > 0) {
            uint256 pending = (user.amount * accRewardPerShare) / ACC_PRECISION - user.rewardDebt;
            if (pending > 0) {
                pending = _safePay(beneficiary, pending);
    ...
    function unstakeFor(address beneficiary, uint256 amount) external nonReentrant whenNotPaused onlyMigrator {
        require(beneficiary != address(0), "NFTStakerV2: zero beneficiary");
        require(amount > 0, "NFTStakerV2: zero unstake");
        UserInfo storage user = users[beneficiary];
        require(user.amount >= amount, "NFTStakerV2: insufficient stake");
        ...
        emit UnstakedFor(msg.sender, beneficiary, amount);
        _recomputeSchedule();
    }

    function withdrawRewardToken(address to, uint256 amount) external onlyOwnerOrMigrator {
        require(totalStaked == 0, "NFTStakerV2: stake outstanding");
        ...
        // When totalStaked == 0 there are no live claimants and committedDebt
        // is expected to be 0 (no pending accrual). Re-derive rewardBudget from
        // actual on-chain balance and force committedDebt to 0 as defence in
        // depth so the strong invariant balance == rewardBudget + committedDebt
        // holds post-sweep.
    ...
    /// @dev Transfers `amount` of rewardToken to `recipient`. Reverts if the
    ...
    function _safePay(address recipient, uint256 amount) internal returns (uint256) {
        require(rewardToken.balanceOf(address(this)) >= amount, "NFTStakerV2: insufficient reward balance");
        if (amount > 0) {
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        require(amount > 0, "NFTStakerV2: nothing to withdraw");
