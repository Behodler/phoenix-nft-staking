...
3. **APY-driven emission rate**: `rewardRate = T * targetAPY / SECONDS_PER_YEAR` where `T` is the closed-form aggregate value of all minted NFTs in the configured dispatcher (read from `nftMinter.configs(dispatcherIndex)` and `nftMinter.totalSupply(stakedId)` via the local `INFTSupply` interface). `T` uses the geometric-series average with ratio `r = 1 + growthBasisPoints/10_000`; computed via `solady FixedPointMathLib.rpow(r, N, 1e18)` and OZ `Math.mulDiv` with floor rounding (conservative for a reward contract). Edge cases: `N == 0 || targetAPY == 0` → `R = 0`; `N == 1` → `T = latestPrice` exactly; `growthBasisPoints == 0` → `T = price * N` (avoids divide-by-zero on `r - 1`).
...
10. **Events and view functions**: Emit `Staked`, `Unstaked`, `Claimed`, `EmergencyWithdrawn`, `Pulled(inflow, newBudget)`, `ToppedUp(from, amount, newBudget)`, `ScheduleRecomputed(T, V, newRate, newWindowEnd)`, `TargetAPYChanged`, `DispatcherIndexChanged`, `NFTMinterChanged`, `DispatcherHookChanged`, `StakedIdChanged`, `PauserChanged`. Expose `pendingReward(user)`, `currentRewardRate()`, `totalDebt()`, `totalBudget()`, `runwaySeconds()`.

### Critical Invariants to Preserve in Tests

- **APY stability under repeated pulls/top-ups within a block**: `R = T * A / SECONDS_PER_YEAR` depends only on on-chain state (`nftMinter.configs`, `nftMinter.totalSupply`, `targetAPY`), not on inflow cadence or size. Calling `pullAndRefresh()` or `topUp(small)` with unchanged `T` and `A` must leave `rewardRate` unchanged.
- **Runway = V / R**: After every recompute, `windowEnd = block.timestamp + V / rewardRate` (with floor division).
- **APY-as-floor for median NFT**: Because `T` is the aggregate value and `N_a` is the average (geometric-mean-adjusted), the median NFT has price ≤ `N_a`. A staker holding the median or lower-priced NFT therefore sees at least the target APY on their staked value. (Holding period × rate on totalStaked's share — see `NFTStakerSustainability.t.sol`.)
...
- **Solvency (at recompute boundaries)**: `balance == rewardBudget + totalDebt` holds immediately after `_recomputeSchedule`. Between recomputes, `_updatePool` decrements `rewardBudget` while `balance` stays constant (modulo `_safePay` decrements), and `_safePay` reverts on any shortfall rather than silently capping payouts — so the contract never pays out more than `balance` and earned rewards are never silently forfeited.
