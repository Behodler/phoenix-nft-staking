...
5. **Window reset on inflow**: After `pull()` (or an owner top-up), if the contract's phUSD balance increased, reset the remaining window to the full configured depletion duration and recompute the per-second rate over the new balance. **If `pull()` produced zero new phUSD, do not reset** — emissions continue against the existing schedule. User-path pulls (through `stake` / `unstake` / `claim`) are rate-limited by `minPullInterval`: once a pull attempt stamps `lastPullAt`, subsequent user-path `_syncBudget` calls silently no-op (no hook call, no schedule reset) until `block.timestamp >= lastPullAt + minPullInterval`. Owner paths (`pullAndRefresh`, `topUp`) bypass the cooldown so the owner retains unconditional control over the schedule. This rate-limit is the M-01 mitigation: the zero-stake EOA grief that pinned the rate by spamming `claim()` each block is neutralised at the mutation site. `minPullInterval` defaults to `0` (pre-fix pull-on-every-interaction behavior) and must be set post-deploy via `setMinPullInterval`.
...
7. **Owner manual phUSD injection**: `topUp` is `onlyOwner`. The owner transfers phUSD into the contract as a top-up, which resets the window the same way a successful `pull()` does. `topUp` does not invoke `_syncBudget` and does not touch `lastPullAt`, so it neither pulls from the dispatcher hook nor disturbs the user-path cooldown clock.
