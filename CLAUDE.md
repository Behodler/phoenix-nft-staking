...
5. **Window extension on inflow — monotonic-upward `rewardRate` invariant**: Every budget-augmenting path (`_syncBudget`'s dispatcher pull and `topUp`'s owner transfer) funnels through the shared `_applyAsymmetricWindow(inflow)` internal. The invariant it enforces: `rewardRate` is monotonic non-decreasing. The reset branch (full schedule refresh at `rewardBudget / windowDuration` and `windowEnd = now + windowDuration`) fires only on bootstrap, post-expiry restart (`block.timestamp >= windowEnd`), or a legitimate rate raise (`candidate > rewardRate`); otherwise the inflow is absorbed as extra budget and `windowEnd` is extended at the current rate by `inflow / rewardRate`. **If `pull()` produced zero new phUSD, do not reset** — emissions continue against the existing schedule. Because the helper never lowers `rewardRate`, rate compression from the M-01 audit finding is structurally impossible on every budget-augmenting path: no cadence-dependent cooldown, no rate-limit state, no griefer-exploitable surface. A zero-stake EOA spamming `claim()` (or an owner spamming small `topUp`s) can only extend the window, never shrink the rate.
...
7. **Owner manual phUSD injection**: `topUp` is `onlyOwner`. The owner transfers phUSD into the contract and the same monotonic-upward invariant applies: small top-ups preserve `rewardRate` and extend `windowEnd` at the current rate; large top-ups (candidate > current rate) raise `rewardRate` and reset `windowEnd`; bootstrap and post-expiry top-ups hit the reset branch. `topUp` does **not** call `_syncBudget`, so it neither pulls from the dispatcher hook nor couples its liveness to the hook's health — the escape-hatch property is preserved if the dispatcher is ever broken.
