...
contract NFTStakerERC1155Test is Test {
    ...
    function setUp() public {
        ...
        staker = new NFTStaker(IERC1155(address(nft)), STAKED_ID, IERC20(address(phUSD)), owner);

        // Mint stakedId to alice + foreign id to alice
    ...
    function testStakeAcceptsConfiguredIdOnly() public {
        // Sanity: stake on STAKED_ID works (no time-based reward needed)
        vm.prank(alice);
        staker.stake(5);
    ...
    function testForeignIdTransferIsAcceptedButNotAccounted() public {
        // alice directly transfers FOREIGN_ID into the staker via safeTransferFrom
        vm.prank(alice);
